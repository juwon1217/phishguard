...
- **Paired Analysis Engine**: 사용자의 답변만 분석하는 것이 아니라, **[AI의 질문 + 사용자의 답변]**을 쌍(Pair)으로 분석하여 맥락을 파악합니다.
- **생성형 피드백 (GenAI Correction)**: Gemini 2.5 Pro가 상황에 맞는 올바른 대처법을 생성합니다.
    - **문제점 지적**: *"모르는 상대에게 비밀번호를 직접 말하는 것은 위험합니다."* (1문장 요약)
    - **모범 답안 제시**: *"아빠, 내가 지금 전화해서 직접 확인해볼게."* (시나리오별 어조 반영 - 반말/존댓말 자동 변환)
- **발동 조건**: 유출 위험도(Leakage Score)가 **40% 이상**일 때 자동으로 심층 분석을 수행합니다.
